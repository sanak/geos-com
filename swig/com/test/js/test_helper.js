...
GeosTestHelper.prototype.createPoint = function(x, y) {
	var cs = geos.CoordinateSequence.new_CoordinateSequence(1, 2);
	cs.setX(0, x);
	cs.setY(0, y);
	return geos.createPoint(cs);
};

GeosTestHelper.prototype.createUshapedLineString = function(xoffset, yoffset, side) {
	var cs = geos.CoordinateSequence.new_CoordinateSequence(4, 2);
	cs.setX(0, xoffset);
	cs.setY(0, yoffset);
	cs.setX(1, xoffset);
	...
	cs.setY(3, yoffset);
	return geos.createLineString(cs);
};

// This function will create a LinearRing geometry
// representing a square with the given origin and side 
GeosTestHelper.prototype.createSquareLinearRing = function(xoffset, yoffset, side) {
	...
	cs.setX(4, xoffset);
	cs.setY(4, yoffset);
	return geos.createLinearRing(cs);
};
...
GeosTestHelper.prototype.printGeoms = function(geoms) {
	var geom
	for (geom in geoms) {
		var wkt = geos.geomToWkt(geom);
		WScript.Echo(wkt);
	}
};
